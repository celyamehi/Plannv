...
export default function ProSidebar({ children }: ProSidebarProps) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar Desktop */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gray-900 text-white transition-all duration-300 hidden lg:flex lg:flex-col`}>
        {/* Header */}
        <div className="h-16 bg-gray-800 flex items-center justify-between px-4 flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            {sidebarOpen && (
              <span className="text-lg font-semibold">PlannV Pro</span>
            )}
          </div>
          <Button
            variant="ghost"
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 lg:hidden" onClick={() => setMobileMenuOpen(false)}>
          <div className="fixed left-0 top-0 h-full w-64 bg-gray-900 text-white z-50" onClick={(e) => e.stopPropagation()}>
