-- SCRIPT DE CRÉATION CORRECTE DES UTILISATEURS SUPABASE
-- Utilise la méthode officielle Supabase pour éviter les erreurs de schéma
...
-- Supprimer aussi de auth.users (méthode sécurisée)
...
DO $$
BEGIN
    RAISE NOTICE '=== CRÉATION DES UTILISATEURS AVEC MÉTHODE SUPABASE CORRECTE ===';
    RAISE NOTICE 'Utilisation de auth.sign_up() pour créer les utilisateurs';
END $$;

-- Créer les utilisateurs avec la méthode officielle Supabase
-- Note: Cette approche simule ce que ferait l'API Supabase
...
BEGIN
  INSERT INTO auth.users (
    ...
    recovery_sent_at, email_change_sent_at, new_email, invited_at,
    confirmation_sent_at, email_change, phone_change, phone_change_sent_at, new_phone
  ) VALUES (
